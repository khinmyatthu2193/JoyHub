...
const safely = (play: () => void) => {
  try { play(); return true } catch { return false }
}
...
const speak = (message: string, kind: 'correct' | 'incorrect') => {
  if (!('speechSynthesis' in window) || typeof SpeechSynthesisUtterance === 'undefined') throw new Error('Speech unavailable')
